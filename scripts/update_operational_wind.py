...
def list_cptec_files(dir_url: str) -> list[tuple[str, datetime, datetime]]:
    ...
    pattern = re.compile(
        r'href=["\'](WRF_cpt_07KM_(\d{10})_(\d{10})\.grib2\.idx)["\']',
        re.I,
    )
    ...
    for back in range(0, 3):
        ...
        for run_hour in (12, 0):
            ...
            try:
                ...
                continue
            for idx_name, run, valid in files:
                ...
                if valid_delta_h > 2.1:
                    continue
                candidates.append((valid_delta_h, run_age_h, dir_url, idx_name, run, valid))
    if not candidates:
        raise SourceUnavailable("nenhum WRF 7 km CPTEC recente com horário válido próximo do momento atual")
    candidates.sort(key=lambda x: (x[0], x[1]))
    _, _, dir_url, idx_name, run, valid = candidates[0]
    return dir_url, idx_name, run, valid
...
def only_field(path: Path) -> Field:
    return read_grib_fields(path)[0][1]
...
def extract_points(u: Field, v: Field, *, max_points: int = MAX_POINTS) -> list[dict]:
    ...
    mask = (
        np.isfinite(u.lat) & np.isfinite(u.lon) & np.isfinite(u.val) & np.isfinite(v.val)
        & (u.lat >= BBOX["south"]) & (u.lat <= BBOX["north"])
        & (u.lon >= BBOX["west"]) & (u.lon <= BBOX["east"])
        & (np.abs(u.val) < 100) & (np.abs(v.val) < 100)
    )
    ...
    out: list[dict] = []
    for i in idx:
        uu = float(u.val[i]); vv = float(v.val[i])
...
def cptec_payload(now: datetime) -> dict:
    dir_url, idx_name, run, valid = choose_cptec_candidate(now)
    idx_url = dir_url + idx_name
    grib_name = idx_name.removesuffix(".idx")
    grib_url = dir_url + grib_name

    try:
        r = http_get(idx_url)
    except requests.RequestException as e:
        raise SourceUnavailable(f"falha ao baixar índice CPTEC: {e}") from e
    if r.status_code != 200:
        raise SourceUnavailable(f"índice CPTEC HTTP {r.status_code}")
    offsets = parse_idx(r.text, grib_url)

    with tempfile.TemporaryDirectory() as td:
        td = Path(td)
        upath = td / "u10.grib2"; vpath = td / "v10.grib2"
        ranged_download(grib_url, *offsets["u"], upath)
        ranged_download(grib_url, *offsets["v"], vpath)
        points = extract_points(only_field(upath), only_field(vpath))
...
def ecmwf_payload(now: datetime, cptec_error: str) -> dict:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            result = client.retrieve(
                date=run.strftime("%Y%m%d"), time=run.hour, type="fc", step=step,
                param=["10u", "10v"], target=str(target),
            )
        except Exception as e:
    ...
    for short, field in fields:
        s = short.lower()
        if s in {"10u", "u10"}: u = field
        elif s in {"10v", "v10"}: v = field
    ...
    if isinstance(result_dt, datetime):
        run = result_dt.replace(tzinfo=timezone.utc) if result_dt.tzinfo is None else result_dt.astimezone(timezone.utc)
        valid = run + timedelta(hours=step)
...
def current_file_is_still_usable(now: datetime) -> bool:
    ...
    try:
        ...
        if not data.get("available") or not data.get("operational"): return False
